    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'tipo_pago' => [
                'required',
                'string',
                Rule::in(['maquina', 'dinheiro', 'vale', 'faturar']),
            ],
            'vale_user_id' => ['nullable', 'integer', 'exists:users,id'],
            'valor_pago' => ['nullable', 'numeric', 'min:0'],
        ]);

        if ($validated['tipo_pago'] === 'vale' && empty($validated['vale_user_id'])) {
            throw ValidationException::withMessages([
        ...
        $dateTime = Carbon::now();
        $isPaid = in_array($validated['tipo_pago'], ['maquina', 'dinheiro'], true);
        $valeUserId = $validated['tipo_pago'] === 'vale' ? $validated['vale_user_id'] : null;
        $valeUserName = null;

        if ($valeUserId) {
            $valeUserName = User::select('id', 'name')->find($valeUserId)?->name;
        }
        ...
        if ($validated['tipo_pago'] === 'dinheiro' && ($valorPago === null || $valorPago <= 0)) {
            throw ValidationException::withMessages([
                'valor_pago' => 'Informe o valor recebido em dinheiro.',
            ]);
        }

        if ($validated['tipo_pago'] !== 'dinheiro') {
            $valorPago = null;
        }

        $troco = 0;
        $cardComplement = 0;
        if ($validated['tipo_pago'] === 'dinheiro' && $valorPago !== null) {
        ...
        $payment = VendaPagamento::create([
            ...
            'tipo_pagamento' => $validated['tipo_pago'],
