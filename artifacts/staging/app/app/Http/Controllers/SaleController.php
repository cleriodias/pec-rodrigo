...
class SaleController extends Controller
{
        ...
        try {
            ...
            return response()->json([
                'sale' => [
                    ...
                    'fiscal_emergency_available' => ! $invoice && $this->supportsEmergencyFiscalPaymentType($storedPaymentType),
                ],
            ]);
    ...
    public function emergencyTransmitFiscalInvoice(
        Request $request,
        ...
        $payment->loadMissing('vendas.produto', 'vendas.unidade', 'notaFiscal');
        $unitId = (int) ($payment->vendas->first()?->id_unidade ?? 0);

        if ($activeUnitId === null || $unitId !== $activeUnitId) {
            abort(403, 'A venda nao pertence a loja ativa do caixa.');
        }

        if (! $this->supportsEmergencyFiscalPaymentType($payment->tipo_pagamento)) {
            return response()->json([
                'message' => 'Esta forma de pagamento nao gera nota fiscal automatica para transmissao.',
        ...
        $currentInvoice = $payment->notaFiscal;
        ...
        try {
            $refreshedInvoice = $fiscalInvoicePreparationService->prepareForPayment(
                ...
                forceFiscalSignature: true,
                refreshFiscalConfiguration: true,
                bypassFiscalGenerationDisabled: true,
            );
            $currentInvoice = $refreshedInvoice ?? $currentInvoice;

            if (! $refreshedInvoice) {
                return response()->json([
                    'message' => 'Nao foi possivel preparar a nota fiscal emergencial desta venda.',
                    'fiscal' => $this->buildFiscalSummary($currentInvoice),
                ], 422);
            }
        ...
        return response()->json([
            'message' => sprintf('Nota fiscal emergencial da venda %d enviada para a SEFAZ.', (int) $notaFiscal->tb4_id),
            'fiscal' => $this->buildFiscalSummary($notaFiscal),
        ]);
    }
    ...
    private function supportsEmergencyFiscalPaymentType(?string $paymentType): bool
    {
        return in_array((string) $paymentType, [
            'dinheiro',
            'cartao_credito',
